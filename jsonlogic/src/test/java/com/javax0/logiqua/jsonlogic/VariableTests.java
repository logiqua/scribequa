...
public class VariableTests {
    ...
    public void testComplexAccess() throws JsonLogicException {
        ...
        assertEquals("John", jsonLogic.apply("""
                {"var": "users.0.name"}
                """, data));
        assertEquals(1337, jsonLogic.apply("""
                {"var": "users.0.followers"}
                """, data));
        assertEquals("Jane", jsonLogic.apply("""
                {"var": "users.1.name"}
                """, data));
        assertEquals(2048, jsonLogic.apply("""
